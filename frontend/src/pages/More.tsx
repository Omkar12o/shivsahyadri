...
function RowItem({ row }: { row: Row }) {
  return (
    ...
    >
      <span className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0', row.danger ? 'bg-red-600/10 text-red-600' : 'bg-saffron/10 text-saffron')}>
        {row.icon}
      </span>
...
export default function More() {
  ...
  if (donation) {
    mandalRows.push({ label: t('nav.donation'), to: '/donation', icon: <Heart className="w-5 h-5" aria-hidden="true" /> })
  }
  ...
  const accountRows: Row[] = [
    { label: 'My Profile', to: '/member/profile', icon: <UserIcon className="w-5 h-5" aria-hidden="true" /> },
  ]
  if (isAdmin) {
    accountRows.push({ label: 'Admin Panel', to: '/admin/dashboard', icon: <Settings className="w-5 h-5" aria-hidden="true" /> })
  }
  ...
  return (
    <div className="app-container py-4 pb-10 md:py-6 lg:py-8">
      <h1 className="text-xl md:text-2xl font-bold text-gray-900">More</h1>
      ...
      <div className="mt-4 lg:grid lg:grid-cols-2 lg:gap-5">
        ...
        <div className="space-y-4">
          ...
          {mandal?.established_year && (
            <div id="abo-mandal" className="card p-5 text-center">
        ...
        {/* Right column — menu groups */}
        <div className="space-y-4 mt-4 lg:mt-0">
          <div className="card divide-y divide-gray-100 overflow-hidden">
            <div className="px-4 pt-3 pb-2 text-[11px] font-bold uppercase tracking-wider text-gray-400">Mandal</div>
            {mandalRows.map((row) => <RowItem key={row.label} row={row} />)}
          </div>
          ...
          <div className="card divide-y divide-gray-100 overflow-hidden">
            <div className="px-4 pt-3 pb-2 text-[11px] font-bold uppercase tracking-wider text-gray-400">Account</div>
            {accountRows.map((row) => <RowItem key={row.label} row={row} />)}
          ...
          <div className="card divide-y divide-gray-100 overflow-hidden">
            <div className="px-4 pt-3 pb-2 text-[11px] font-bold uppercase tracking-wider text-gray-400">Preferences</div>
            <button
              type="button"
      ...
      <p className="text-center text-xs text-gray-400 pt-8">Shivsaydri Ganesh Mandal • Private Members App</p>
    </div>
  )
}